...
!BOP
!
! !MODULE: Merra2_A3MstCModule
!
! !DESCRIPTION: Module Merra2_A3MstCModule contains routines to create the 
!  GEOS-Chem average 3-hr data files (moist parameters on level centers) 
!  from the MERRA2 raw data.
!\\
!\\
! !INTERFACE: 
...
100 FORMAT ( '%%% Defining netCDF file vars & attrs for ', a' grid' )
    ...
    ! Title string
    lName = 'MERRA2 time-averaged 3-hour moist parameters on level centers (A3mstC), processed for GEOS-Chem input'
...
!BOP
!
! !IROUTINE: Merra2_MakeA3MstC
!
! !DESCRIPTION: Routine Merra2_MakeA3MstC is the the driver routine for 
! \begin{enumerate}
! \item Extracting 3-hr time-averaged data fields (moist parameters on level
!       centers) from the MERRA2 raw data files (netCDF-4 format),
...
!
    ...
    ! Echo info    
    msg = '%%%%%%%%%% ENTERING ROUTINE Merra2_MakeA3MstC %%%%%%%%%%'
    WRITE( IU_LOG, '(a)' )
    WRITE( IU_LOG, '(a)' ) TRIM( msg )
...
110 FORMAT( '%%% TOTAL # OF FIELDS TO BE REGRIDDED : ', i5 )
    ...
    ! Echo info
    msg = '%%%%%%%%%% LEAVING ROUTINE Merra2_MakeA3MstC %%%%%%%%%%'
    WRITE( IU_LOG, '(a)' ) '%%%'   
    WRITE( IU_LOG, '(a)' ) TRIM( msg )

  END SUBROUTINE Merra2_MakeA3MstC
!EOC
!------------------------------------------------------------------------------
!                  GEOS-Chem Global Chemical Transport Model                  !
!------------------------------------------------------------------------------
...
!
! !DESCRIPTION: Subroutine Process3dMstNv regrids the MERRA2 met fields from 
...
!
    ...
    REAL*4,  TARGET         :: FC   ( I05x0625, J05x0625, L05x0625 )
    REAL*4,  TARGET         :: FL   ( I05x0625, J05x0625, L05x0625 )
    ...
    ! Loop over the number of times per day
    DO H = 1, TIMES_A3
...
!EOC
END MODULE Merra2_A3MstCModule
